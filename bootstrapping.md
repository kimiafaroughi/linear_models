...
``` r
#nonconstant variance violates linear regression assumptions, but bootstrapping can repeatedly sample to obtain more valid estimates
```
...
boot_sample = function(df) {
  ...
  sample_frac(df, size = 1, replace = TRUE) #gives sample of same sample size but different ones each time
...
I want to formalize this a bit and extract results
...
bootstrap_results = 
  sim_df_nonconst |> 
  bootstrap(n = 5000) |> #generates all bootstrap samples
  mutate(
...
``` r
#bootstrapping gives the right results when assumptions are met and when they are not met
```
...
``` r
#data is super skewed, assumptions likely aren't met
```
...
airbnb_bootstrap_results =
  ...
  bootstrap(n = 1000) |>  #always start small and then change as needed
  mutate(
    df = map(strap, as_tibble),
